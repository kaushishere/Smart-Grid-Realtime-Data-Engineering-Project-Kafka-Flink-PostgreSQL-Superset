...
public class DataStreamJob {
	...
	public static void main(String[] args) throws Exception {
        ...
        KafkaSource<Reading> source = KafkaSource.<Reading>builder()
         ...
         DataStreamSource<Reading> readingStream = env.fromSource(source, WatermarkStrategy.noWatermarks(), "Kafka Source");

         // filter out healthy grid events
//        DataStream<Reading> unhealthyGridEvents = readingStream.filter(DataStreamJob::filterOutHealthyGridEvents);
//        unhealthyGridEvents.print();

        // count by fault type
        DataStream<Tuple2<String, Integer>> counts = readingStream
                 .map(DataStreamJob::countByFault)
                         .keyBy(t->t.f0)
                                 .sum(1);
